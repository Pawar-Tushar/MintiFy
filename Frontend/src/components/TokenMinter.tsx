...
//     getMint,
//     // TokenAccountNotFoundError, // Import specific error if needed
...
//             const amountInBaseUnits = BigInt(Math.floor(parsedAmount * Math.pow(10, mintInfo.decimals))); // Use floor to avoid fractional BigInts
...
//              const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash('confirmed'); // Use confirmed blockhash
...
//             toast.success( 
//                 `${currentAmountInput} tokens minted!`
//             );
...
//                  if (error.message.toLowerCase().includes("mint authority mismatch") || error.message.includes("owner does not match") || error.message.includes("Missing signature for instruction") || error.message.includes("authority") ) {
...
//                 } else if (error.message.includes("Invalid mint") || error.message.includes("Could not find mint")) {
//                     errorMessage = "Invalid Token Mint address. Please double-check."
//                  } else if (error.logs) {
//                     const logMessages = error.logs.join('\n');
...
//                     } else {
//                        errorMessage += ` Logs: ${logMessages.substring(0, 150)}${logMessages.length > 150 ? '...' : ''}`; // Show start of logs
//                      }
...
//             <h3 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">2. Mint Tokens</h3>
//             <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
//                 Increase the supply of an existing SPL token. You must be the designated "Mint Authority" for the token. Minted tokens will go to your wallet's associated account for this token ({network}).
//             </p>
//             <div className="mb-4">
//                 <label htmlFor="mintAddress" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
//                     Token Mint Address:
//                 </label>
//                 <input
//                     id="mintAddress"
...
//                 }`}
//             >
//                {isLoading ? ( <div className="flex justify-center items-center"> <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"> <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle> <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path> </svg> Minting... </div>) : 'Mint Tokens'}
//             </button>


//             {lastTxSig && mintedAmountDisplay && ( 
//                 <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/50 rounded-lg border border-green-300 dark:border-green-700 shadow-sm">
//                     {/* 1. Clear Confirmation Heading */}
//                     <h4 className="text-lg font-semibold text-green-800 dark:text-green-100 mb-3 flex items-center">
//                          <svg className="w-6 h-6 mr-2 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
//                          Minting Successful!
//                     </h4>

//                     <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
//                          Successfully minted{' '}
//                          <span className="font-semibold text-gray-800 dark:text-gray-100">{mintedAmountDisplay}</span>{' '}
//                          tokens{' '}
...
//                      <div className="pt-3 border-t border-green-200 dark:border-green-600">
